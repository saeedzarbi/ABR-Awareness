...
"""
ادامه training از checkpoint_300
"""
import os
import sys
import torch
...
def continue_training():
    """ادامه training از checkpoint موجود"""
    ...
    CONFIG = {
        ...
        # Training (بهبود یافته)
        'total_timesteps': 500_000,      # ✅ 500K timestep اضافه
        'batch_size': 64,
        'n_epochs': 4,
        'lr': 1e-4,                      # ✅ کاهش learning rate (fine-tuning)
        'gamma': 0.99,
        'gae_lambda': 0.95,
        'clip_epsilon': 0.15,            # ✅ کاهش clip (stable تر)
        'entropy_coef': 0.05,            # ✅ کاهش entropy (کمتر explore)
        'value_coef': 0.5,
        'max_grad_norm': 0.5,
        ...
        # Output
        'output_dir': 'results/fcc_training_continued',
        'checkpoint_to_load': 'results/fcc_training/checkpoint_300.pth'  # ✅
    }
    ...
    print(f"   ✅ Loaded from: {CONFIG['checkpoint_to_load']}")
    print(f"   Previous update: {checkpoint.get('update', 'N/A')}")
    ...
    if 'optimizer_state_dict' in checkpoint:
        try:
            ...
            print("   ⚠️  Could not restore optimizer state, using fresh optimizer")
    
    # Create logger
    logger = TrainingLogger(
        log_dir='results/logs',
        run_name='fcc_training_continued'
    )
    ...
    # Training loop
    print("\n" + "="*70)
    print("🚂 Continuing Training (Fine-tuning)")
    print("="*70)
    print(f"Additional timesteps: {CONFIG['total_timesteps']:,}")
    print(f"Lower LR: {CONFIG['lr']} (fine-tuning mode)")
    print(f"Evaluation every: {CONFIG['eval_interval']} updates")
    print("="*70 + "\n")
    ...
    while timestep < CONFIG['total_timesteps']:
        ...
        if update_count % CONFIG['eval_interval'] == 0:
            ...
            if val_mean > best_reward + 1.0:
                ...
                no_improvement += 1
                print(f"   ⚠️  No improvement ({no_improvement}/{CONFIG['early_stopping_patience']})")
