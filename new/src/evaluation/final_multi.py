...
class TCSVT_Evaluator:
    ...
    def load_methods(self):
        ...
        # 1. Proposed (V10)
        try:
            path = PATHS['models'] / 'ppo_abr_multi_dynamic_14' / 'best_model' / 'best_model'
            if not path.with_suffix('.zip').exists():
                path = PATHS['models'] / 'ppo_abr_multi_dynamic_14' / 'final_model'
    ...
    def evaluate(self, methods, episodes_per_video=20, enable_logging=True):
        print(f"\n🔬 Running Evaluation V14 (N={episodes_per_video} per video)...")
        print(f"   Enhanced logging: {'Enabled' if enable_logging else 'Disabled'}")
        
        send_slack_message("info", "Evaluation Started", 
                          f"Starting V14 evaluation on {len(self.test_videos)} videos")
        ...
        for video_idx, video_name in enumerate(self.test_videos, 1):
            ...
            for name, model in methods.items():
                ...
                if enable_logging:
                    logger = EvaluationLogger(PATHS['logs'] / 'evaluation_v14')
    ...
    def save_statistics(self):
        ...
        path = PATHS['results'] / 'detailed_stats_multi_video_14.csv'
    ...
    def print_summary(self, df):
        ...
        # Send to Slack
        details_msg = "📊 *Final Evaluation Results V14*\n\n"
