...
class TCSVT_Evaluator:
    ...
    def evaluate(self, methods, video_name='park_joy', episodes=50):
    ...
    def save_statistics(self):
        ...
        df.to_csv(PATHS['results'] / 'detailed_stats_multi_video.csv', index=False)
    ...
    def load_and_calculate_statistics(self, csv_file='detailed_stats_multi_video.csv'):
        """Load existing CSV file and calculate statistics"""
        csv_path = PATHS['results'] / csv_file
        
        if not csv_path.exists():
            print(f"❌ File not found: {csv_path}")
            return None
        
        print(f"\n📊 Loading statistics from: {csv_file}")
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Evaluate ABR methods or load existing results')
    parser.add_argument(
        '--load',
        type=str,
        default=None,
        help='Load and calculate statistics from existing CSV file (e.g., detailed_stats_multi_video.csv)'
    )
    parser.add_argument(
        '--episodes',
        type=int,
        default=50,
        help='Number of episodes to run (if not loading from file)'
    )
    ...
    if args.load:
        # Load from existing CSV and calculate statistics
        evaluator.load_and_calculate_statistics(csv_file=args.load)
    else:
        # Run evaluation
        methods = evaluator.load_methods()
        if methods:
            evaluator.evaluate(methods, episodes=args.episodes)
