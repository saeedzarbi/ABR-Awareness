"""
Master evaluation script (v12) for all methods.

Outputs:
  - detailed_stats_master_v12<suffix>.csv
  - decision_log_v12<suffix>.csv
  - <method>_vs_genie_v12<suffix>.csv

V12 evaluation principle:
- If a method is trained/deployed with wrappers (shield/hysteresis), evaluation
  MUST apply the same wrappers. Otherwise metrics are meaningless.
"""
...
MODEL_DIRS = {
    "Proposed": "proposed_v12",
    "Proposed_Shielded": "proposed_shielded_v12",
    "Proposed_ShieldedQoE": "proposed_shielded_qoe_v12",
    "Proposed_ShieldedRiskGate": "proposed_shielded_riskgate_v12",
    "Ablation_Base": "ablation_base_v12",
    "Ablation_Future": "ablation_future_v12",
    "Ablation_Lyap": "ablation_lyap_v12",
    "Pensieve": "pensieve_v12",
}
...
def load_rl_model(display_name: str, folder_name: str):
    preferred = PATHS["models"] / "master_v12" / folder_name
    resolved = _resolve_model_path(preferred)
    if resolved is None:
        raise FileNotFoundError(f"No v12 model found for {display_name} at {preferred}")
...
def _shield_cfg_from_env(risk_gate: bool = False) -> ShieldConfig:
    ...
    try:
        ...
        risky_ratio = 1.10
    # Enable risk gate if the method is risk-gated, or a global env var requests it.
    only_when_risky = risk_gate or os.environ.get("ABR_V12_RISK_GATE", "0").strip().lower() in {"1", "true", "yes"}
...
def _wrap_method_env(method_name: str, env):
    """
    Ensure evaluation matches the deployed method wrappers.
    """
...
def _applied_action(info: dict, fallback_action: int) -> int:
    # Hysteresis wrapper sets 'applied_action'; shield sets 'shielded_action'
...
def run_eval(episodes_per_video: int = 20, suffix: str = ""):
    ...
    out_csv = PATHS["results"] / f"detailed_stats_master_v12{suffix}.csv"
    df.to_csv(out_csv, index=False)
    print(f"\nSaved episode results : {out_csv}")

    df_chunks = pd.DataFrame(chunk_decisions)
    decisions_csv = PATHS["results"] / f"decision_log_v12{suffix}.csv"
    ...
    if sp_stats is not None and "Genie" in df["Method"].values:
        ...
        for m in sorted([x for x in df["Method"].unique() if x != "Genie"]):
            ...
            try:
                ...
                print(f"  {m:26s}: n={len(paired):3d} p={pval:.4f} {sig}")
            except Exception:
                pass
...
def _build_comparison(df_chunks: pd.DataFrame, method_name: str, suffix: str = ""):
    ...
    cmp_csv = PATHS["results"] / f"{safe_name}_vs_genie_v12{suffix}.csv"
