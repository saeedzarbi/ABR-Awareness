"""
Master evaluation script (v8) for all methods.

V8 writes outputs to versioned filenames:
  - detailed_stats_master_v8<suffix>.csv
  - decision_log_v8<suffix>.csv
  - proposed_vs_genie_v8<suffix>.csv

Safety guard fairness:
  - Default applies guard only to RL methods (ABR_GUARD_SCOPE=rl).
  - Use --guard-scope all to treat the guard as a shared system component.

Statistical tests:
  - If SciPy is available, Wilcoxon signed-rank is computed PAIRWISE
    vs Genie by (Video, Episode).
"""
...
MODEL_DIRS = {
    "Proposed": "proposed_v8",
    "Ablation_Base": "ablation_base_v8",
    "Ablation_Future": "ablation_future_v8",
    "Ablation_Lyap": "ablation_lyap_v8",
    "Pensieve": "pensieve_v8",
}
...
def load_rl_model(display_name: str, folder_name: str):
    preferred = PATHS["models"] / "master_v8" / folder_name
    resolved = _resolve_model_path(preferred)
    if resolved is None:
        raise FileNotFoundError(f"No v8 model found for {display_name} at {preferred}")
...
def run_eval(episodes_per_video: int = 20, suffix: str = "", guard_scope: Optional[str] = None):
    ...
    for video_name in test_videos:
        ...
        for name, model in methods.items():
            ...
            eval_env = env
            if name == "Pensieve":
                eval_env = ContentBlindWrapper(env)
            ...
            for ep in range(episodes_per_video):
                ...
                while not done:
                    ...
                    if _safety_guard_level() != "off" and _should_apply_guard(name):
    ...
    out_csv = PATHS["results"] / f"detailed_stats_master_v8{suffix}.csv"
    df.to_csv(out_csv, index=False)
    print(f"\nSaved episode results : {out_csv}")

    df_chunks = pd.DataFrame(chunk_decisions)
    decisions_csv = PATHS["results"] / f"decision_log_v8{suffix}.csv"
    ...
    # Paired Wilcoxon vs Genie
    if sp_stats is not None and "Genie" in df["Method"].values:
        print("\n--- Wilcoxon signed-rank test vs Genie (QoE, paired by Video+Episode, v8) ---")
...
def _build_comparison(df_chunks: pd.DataFrame, suffix: str = ""):
    ...
    cmp_csv = PATHS["results"] / f"proposed_vs_genie_v8{suffix}.csv"
