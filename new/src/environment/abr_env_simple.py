...
class ABREnvSimple(gym.Env):
    """
    Simplified ABR Environment
    - No exponential risk factor
    ...
    # 🎯 SIMPLE PENALTIES (no exponential!)
    REBUF_PENALTY = 10.0  # Linear penalty
    SMOOTH_PENALTY = 1.0
    
    def __init__(self, video_names: Union[str, List[str]] = 'bigbuckbunny', 
                 trace_dir='data/standardized/train_traces', 
    ...
    def step(self, action):
        ...
        # 🎯 SIMPLE LINEAR REWARD (no exponential!)
        reward = current_vmaf - (self.REBUF_PENALTY * rebuffer_time) - (self.SMOOTH_PENALTY * smooth_pen)
