...
class ABREnv(gym.Env):
    """
    Multi-Video ABR Environment V2 (Fixed)

    Critical fixes from V1:
    1. self.np_random for ALL randomness → proper Gymnasium seeding & fair evaluation
    2. VBR chunk size simulation → future-awareness observation is now meaningful
    3. Randomized initial conditions → breaks observation symmetry, prevents policy collapse
    4. Reward normalization (/100) → stable PPO gradient scale
    5. Lyapunov / Future toggles preserved for ablation studies
    """
    ...
    def _generate_vbr_profiles(self):
        """Deterministic per-video VBR multipliers (simulates real encoding)."""
        for vid in self.video_names:
    ...
    def get_chunk_size_bits(self, bitrate_kbps, chunk_idx):
        """Public helper for baselines (Genie / Fugu) to query actual sizes."""
