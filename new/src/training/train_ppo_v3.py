...
class TrainingConfigV13:
    ...
    ENT_COEF = 0.15  # V13: Increased exploration         
    VF_COEF = 0.5
    MAX_GRAD_NORM = 0.5
    
    TOTAL_TIMESTEPS = 1_500_000  # V13: More training 
    EVAL_FREQ = 20_000
    SAVE_FREQ = 50_000
...
def make_env(rank: int, seed: int = 0, is_eval: bool = False):
    def _init():
        if is_eval:
            video_list = TrainingConfigV13.TEST_VIDEOS
            trace_path = PATHS['test_traces']
        else:
            video_list = TrainingConfigV13.TRAIN_VIDEOS
        ...
        env = ABREnv(
            ...
            max_chunks=TrainingConfigV13.MAX_CHUNKS,
...
def main():
    ...
    print("="*70)
    print(f"📚 Training Videos: {TrainingConfigV13.TRAIN_VIDEOS}")
    print(f"🧪 Test Videos: {TrainingConfigV13.TEST_VIDEOS}")
    print("\n📊 Configuration:")
    print(f"   REBUF_PENALTY_BASE: 75.0")
    print(f"   Risk Factor Cap: 8.0")
    print(f"   SI/TI Normalization: 150, 70")
    print(f"   Learning Rate: {TrainingConfigV13.LEARNING_RATE}")
    print(f"   Total Timesteps: {TrainingConfigV13.TOTAL_TIMESTEPS:,}")
    print("="*70 + "\n")
    ...
    train_env = SubprocVecEnv([
        ...
        for i in range(TrainingConfigV13.NUM_ENVS)
    ])
    ...
    model = PPO(
        'MlpPolicy',
        train_env,
        learning_rate=TrainingConfigV13.LEARNING_RATE,
        n_steps=TrainingConfigV13.N_STEPS,
        batch_size=TrainingConfigV13.BATCH_SIZE,
        n_epochs=TrainingConfigV13.N_EPOCHS,
        gamma=TrainingConfigV13.GAMMA,
        gae_lambda=TrainingConfigV13.GAE_LAMBDA,
        clip_range=TrainingConfigV13.CLIP_RANGE,
        ent_coef=TrainingConfigV13.ENT_COEF,
        vf_coef=TrainingConfigV13.VF_COEF,
        max_grad_norm=TrainingConfigV13.MAX_GRAD_NORM,
        verbose=1,
        device=TrainingConfigV13.DEVICE,
    ...
    callbacks = CallbackList([
        # Checkpoint saving
        CheckpointCallback(
            save_freq=TrainingConfigV13.SAVE_FREQ // TrainingConfigV13.NUM_ENVS, 
        ...
        EvalCallback(
            ...
            eval_freq=TrainingConfigV13.EVAL_FREQ // TrainingConfigV13.NUM_ENVS, 
            n_eval_episodes=20,
            deterministic=True,
            render=False,
            verbose=1
    ...
    try:
        model.learn(
            total_timesteps=TrainingConfigV13.TOTAL_TIMESTEPS, 
            callback=callbacks, 
            progress_bar=True
        )
