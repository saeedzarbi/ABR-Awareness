"""
Unified training script (V5) for all learning-based models:
- Proposed (Lyapunov + Future + Lagrangian CMDP)
...
V5 changes over V4:
- Environment V5 with balanced reward weights and chunk-0 smooth fix
...
- Entropy coefficient 0.035 for Proposed, 0.03 for others
- CMDP v5.2 tuning (rebuf_target=0.04, smooth_target=4.0,
  lambda_rebuf_init=5.0, lyapunov_weight=0.7): intermediate between
  v5.0 (too conservative) and v5.1 (collapsed to 1200kbps)
...
from src.training.constrained_abr import DualVariableLogger, LagrangianRewardWrapper
...
def train_one_model(model_key: str):
    spec = MODEL_SPECS[model_key]
    model_root = PATHS["models"] / "master_v5" / spec["folder"]
    log_root = PATHS["logs"] / "master_v5" / spec["folder"]
...
def _run_parallel(model_keys: List[str], max_workers: int, gpu_ids: List[int] = None):
    ...
    while remaining or running:
        while remaining and len(running) < max_workers:
            ...
            log_file = PATHS["logs"] / "master_v5" / f"{key}_parallel.log"
...
def main():
    parser = argparse.ArgumentParser(
        description="Train all ABR RL models V5 (CMDP + Lagrangian)."
    )
