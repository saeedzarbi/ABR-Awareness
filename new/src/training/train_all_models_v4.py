"""
Unified training script (V4) for all learning-based models:
...
V4 fixes over V3:
- Uses env V3 with download-time ratio features and corrected observation scaling
- Per-model entropy coefficient and timestep budget
- Larger batch size (512) for more stable gradients
- Outputs under results/models/master_v4/ and results/logs/master_v4/
"""
...
class ContentBlindWrapper(ObservationWrapper):
    """
    Masks content-aware / future signals in ABREnv V3 observation.
    V3 layout (35-dim):
      ...
      14    last bitrate
      15:17 SI / TI
      17:23 VMAF per bitrate
      23:29 next chunk sizes
      29:35 download-time ratios
    Indices 15: are zeroed so Pensieve only sees network + buffer state.
    """
...
def train_one_model(model_key: str):
    spec = MODEL_SPECS[model_key]
    model_root = PATHS["models"] / "master_v4" / spec["folder"]
    log_root = PATHS["logs"] / "master_v4" / spec["folder"]
...
def _run_parallel(model_keys: List[str], max_workers: int, gpu_ids: List[int] = None):
    """Launch up to *max_workers* model trainings simultaneously as separate
    OS processes.  Each gets its own CUDA_VISIBLE_DEVICES when *gpu_ids* is
    provided, otherwise all share the default device."""
    ...
    print(f"\n[PARALLEL] Launching {len(remaining)} models, max {max_workers} concurrent")
    if gpu_ids:
        print(f"[PARALLEL] GPU pool: {gpu_ids}")
    ...
    while remaining or running:
        while remaining and len(running) < max_workers:
            ...
            log_file = PATHS["logs"] / "master_v4" / f"{key}_parallel.log"
            ...
            print(f"[PARALLEL] Started {key.upper()} (PID {proc.pid}){gpu_tag}  → {log_file}")
        ...
        for key in list(running):
            ...
            if ret is not None:
                ...
                if ret == 0:
                    ...
                    print(f"[PARALLEL] {key.upper()} finished successfully")
                else:
                    failed.append(key)
                    print(f"[PARALLEL] {key.upper()} FAILED (exit code {ret})")

        if running:
            time.sleep(15)

    print("\n" + "=" * 72)
    print(f"[PARALLEL] Completed: {completed}")
    if failed:
        print(f"[PARALLEL] FAILED:    {failed}")
        sys.exit(1)
    print("All parallel trainings finished.")
    print("=" * 72)


def main():
    parser = argparse.ArgumentParser(
        description="Train all ABR RL models V4 (Proposed/Ablations/Pensieve)."
    )
    parser.add_argument(
        "--all",
        action="store_true",
        help="Train all 5 models.",
    )
    parser.add_argument(
        "--models",
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Max models to train simultaneously (default 1 = sequential).",
    )
    parser.add_argument(
        "--gpu-ids",
        type=str,
