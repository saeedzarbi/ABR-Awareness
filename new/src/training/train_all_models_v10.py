"""
Unified training script (v10) for all learning-based models (paper-ready).

What v10 changes vs v9:
- Outputs go to master_v10 (models/logs).
- Adds a new method-level safety mechanism: Proposed_ShieldedAdaptive.
  This uses the V10 shield with an adaptive gate so it only intervenes under
  clearly risky conditions (low throughput + low/medium buffer + high action).
  Goal: reduce rebuffer on crowd_run while preserving QoE vs fully shielded.

Run:
  python new/src/training/train_all_models_v10.py --all
"""
...
from src.training.constrained_abr_v10 import DualVariableLogger, LagrangianRewardWrapperV10
...
MODEL_SPECS: Dict[str, Dict] = {
    ...
    # V9-style fully shielded method
    "proposed_shielded": {
        "folder": "proposed_shielded_v10",
        "use_lyapunov": True,
        "use_future": True,
        "blind_features": False,
        "use_lagrangian": True,
    ...
    # NEW: adaptive-gated shield (intervene only when risky)
    "proposed_shielded_adaptive": {
        "folder": "proposed_shielded_adaptive_v10",
        "use_lyapunov": True,
        "use_future": True,
        "blind_features": False,
        "use_lagrangian": True,
...
def _shield_cfg(model_key: str) -> ShieldConfig:
    ...
    mode = spec.get("shield", "off")
    if mode == "off":
        return ShieldConfig(level="off")
    if mode == "adaptive":
        # Only intervene when TP is low and buffer is not high, and policy asks for >=1200
        return ShieldConfig(
            level=level,
    ...
    return ShieldConfig(level=level)
...
def make_env(rank: int, model_key: str, seed: int = 0, is_eval: bool = False):
    ...
    def _init():
        ...
        if spec.get("use_lagrangian") and not is_eval:
            env = LagrangianRewardWrapperV10(env)
...
def train_one_model(model_key: str):
    spec = MODEL_SPECS[model_key]
    model_root = PATHS["models"] / "master_v10" / spec["folder"]
    log_root = PATHS["logs"] / "master_v10" / spec["folder"]
    model_root.mkdir(parents=True, exist_ok=True)
    log_root.mkdir(parents=True, exist_ok=True)

    print("\n" + "=" * 72)
    print(
        f"Training {model_key.upper()} (v10) | lyap={spec['use_lyapunov']} "
    ...
    try:
        ...
        print(f"[DONE] {model_key} v10 training completed.")
    except KeyboardInterrupt:
        model.save(model_root / "interrupted_model")
        print(f"[INTERRUPTED] {model_key} v10 checkpoint saved.")
    finally:
        train_env.close()
        eval_env.close()
...
def _run_parallel(model_keys: List[str], max_workers: int, gpu_ids: Optional[List[int]] = None):
    ...
    print(f"\n[PARALLEL v10] Launching {len(remaining)} models, max {max_workers} concurrent")
    if gpu_ids:
        print(f"[PARALLEL v10] GPU pool: {gpu_ids}")
    ...
    while remaining or running:
        while remaining and len(running) < max_workers:
            ...
            log_file = PATHS["logs"] / "master_v10" / f"{key}_parallel.log"
            ...
            print(f"[PARALLEL v10] Started {key.upper()} (PID {proc.pid}){gpu_tag}  -> {log_file}")
        ...
        for key in list(running):
            ...
            if ret is not None:
                ...
                if ret == 0:
                    ...
                    print(f"[PARALLEL v10] {key.upper()} finished successfully")
                else:
                    failed.append(key)
                    print(f"[PARALLEL v10] {key.upper()} FAILED (exit code {ret})")

        if running:
            time.sleep(15)

    print("\n" + "=" * 72)
    print(f"[PARALLEL v10] Completed: {completed}")
    if failed:
        print(f"[PARALLEL v10] FAILED:    {failed}")
        sys.exit(1)
    print("All parallel v10 trainings finished.")
    print("=" * 72)


def main():
    parser = argparse.ArgumentParser(description="Train all ABR RL models v10 (adaptive shield).")
    parser.add_argument("--all", action="store_true", help="Train all v10 models.")
    parser.add_argument(
        "--models",
        type=parse_model_list,
        default=["proposed", "proposed_shielded", "proposed_shielded_adaptive"],
    ...
    if args.parallel > 1 and len(order) > 1:
        ...
        print("\n" + "=" * 72)
        print("All requested v10 trainings finished.")
        print("=" * 72)
