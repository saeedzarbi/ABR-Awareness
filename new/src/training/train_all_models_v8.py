"""
Unified training script (v8) for all learning-based models.

V8 goal: a clean re-train-from-scratch pipeline with versioned output folders:
  - Models: PATHS["models"]/master_v8/<method_folder>/
  - Logs  : PATHS["logs"]  /master_v8/<method_folder>/

This script intentionally keeps the same SB3 PPO setup as v7, but pins imports
to explicit v8 modules (`abr_multi_env_v8`, `constrained_abr_v8`) to avoid
version drift.
"""
...
from src.environment.abr_multi_env_v8 import ABREnv
from src.training.constrained_abr_v8 import DualVariableLogger, LagrangianRewardWrapperV8
...
def make_env(rank: int, model_key: str, seed: int = 0, is_eval: bool = False):
    ...
    def _init():
        ...
        if spec.get("use_lagrangian") and not is_eval:
            env = LagrangianRewardWrapperV8(env)
...
def train_one_model(model_key: str):
    spec = MODEL_SPECS[model_key]
    model_root = PATHS["models"] / "master_v8" / spec["folder"]
    log_root = PATHS["logs"] / "master_v8" / spec["folder"]
    model_root.mkdir(parents=True, exist_ok=True)
    log_root.mkdir(parents=True, exist_ok=True)

    print("\n" + "=" * 72)
    print(
        f"Training {model_key.upper()} (v8) | lyap={spec['use_lyapunov']} "
    ...
    try:
        ...
        print(f"[DONE] {model_key} v8 training completed.")
    except KeyboardInterrupt:
        model.save(model_root / "interrupted_model")
        print(f"[INTERRUPTED] {model_key} v8 checkpoint saved.")
    finally:
        train_env.close()
        eval_env.close()
...
def _run_parallel(model_keys: List[str], max_workers: int, gpu_ids: Optional[List[int]] = None):
    ...
    print(f"\n[PARALLEL v8] Launching {len(remaining)} models, max {max_workers} concurrent")
    if gpu_ids:
        print(f"[PARALLEL v8] GPU pool: {gpu_ids}")
    ...
    while remaining or running:
        while remaining and len(running) < max_workers:
            ...
            log_file = PATHS["logs"] / "master_v8" / f"{key}_parallel.log"
            ...
            print(f"[PARALLEL v8] Started {key.upper()} (PID {proc.pid}){gpu_tag}  -> {log_file}")
        ...
        for key in list(running):
            ...
            if ret is not None:
                ...
                if ret == 0:
                    ...
                    print(f"[PARALLEL v8] {key.upper()} finished successfully")
                else:
                    failed.append(key)
                    print(f"[PARALLEL v8] {key.upper()} FAILED (exit code {ret})")

        if running:
            time.sleep(15)

    print("\n" + "=" * 72)
    print(f"[PARALLEL v8] Completed: {completed}")
    if failed:
        print(f"[PARALLEL v8] FAILED:    {failed}")
        sys.exit(1)
    print("All parallel v8 trainings finished.")
    print("=" * 72)


def main():
    parser = argparse.ArgumentParser(
        description="Train all ABR RL models v8 (clean retrain-from-scratch)."
    )
    ...
    if args.parallel > 1 and len(order) > 1:
        ...
        print("\n" + "=" * 72)
        print("All requested v8 trainings finished.")
        print("=" * 72)
