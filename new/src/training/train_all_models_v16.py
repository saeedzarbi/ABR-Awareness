"""
Unified training script (V16, per-chunk non-monotone VMAF ladder).

Identical to train_all_models_v14.py except:
  * uses abr_multi_env_v16 (per-chunk multi-resolution VMAF ladder), and
  * saves under models/master_v16_perchunk/.

Standalone (not a monkey-patch): PPO uses SubprocVecEnv and env-class overrides in
the parent process do not reach the pickled env factory in the workers.

Prerequisite: run data/build_multires_vmaf.py first so
data/vmaf_scores/vmaf_perchunk_multires.csv exists (otherwise the env falls back
to the monotone session ladder and VMAF-awareness stays inert).

Run:
  cd new
  python src/training/train_all_models_v16.py --all --seeds 0 --num-envs 8
"""
...
def train_one_model(model_key: str, seed: int, timesteps_scale: float, num_envs: int):
    ...
    print("\n" + "=" * 72)
    print(
        f"Training {model_key.upper()} (v16 per-chunk VMAF) seed={seed} | "
    ...
    try:
        ...
        print(f"[DONE] {model_key} v16 seed={seed} completed.")
    except KeyboardInterrupt:
        model.save(model_root / "interrupted_model")
        print(f"[INTERRUPTED] {model_key} v16 seed={seed} checkpoint saved.")
    finally:
        train_env.close()
        eval_env.close()
...
def _run_parallel(model_keys: List[str], seeds: List[int], max_workers: int,
    ...
    completed, failed = [], []
    print(f"\n[PARALLEL v16] {len(remaining)} (model,seed) jobs, max {max_workers} concurrent")
    while remaining or running:
        while remaining and len(running) < max_workers:
            ...
            running[tag] = (proc, fh)
            print(f"[PARALLEL v16] Started {tag} (PID {proc.pid}) -> {log_file}")
        for tag in list(running):
            ...
            if ret is not None:
                ...
                print(f"[PARALLEL v16] {tag} {'OK' if ret == 0 else f'FAILED ({ret})'}")
        if running:
            time.sleep(15)
    print(f"\n[PARALLEL v16] Completed: {completed}")
    if failed:
        print(f"[PARALLEL v16] FAILED: {failed}")
        sys.exit(1)


def main():
    parser = argparse.ArgumentParser(description="Train all ABR RL models (v16, per-chunk VMAF).")
    ...
    if args.parallel > 1 and n_jobs > 1:
        ...
        print("\nAll requested v16 (per-chunk VMAF) trainings finished.")
