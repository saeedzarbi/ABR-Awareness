"""
Unified training script (v9) for all learning-based models (paper-ready).

What v9 changes vs v8:
- Outputs go to master_v9 (models/logs).
- Adds an explicit method-level safety mechanism: Proposed_Shielded.
  This method trains with a deterministic safety shield in-the-loop, so
  improvements are attributable to the method (not a global evaluation guard).

Run:
  python new/src/training/train_all_models_v9.py --all
"""
...
from src.training.constrained_abr_v9 import DualVariableLogger, LagrangianRewardWrapperV9
...
MODEL_SPECS: Dict[str, Dict] = {
    ...
    # Paper-friendly variant: method-level shield
    "proposed_shielded": {
        "folder": "proposed_shielded_v9",
        "use_lyapunov": True,
        "use_future": True,
        "blind_features": False,
        "use_lagrangian": True,
...
def train_one_model(model_key: str):
    spec = MODEL_SPECS[model_key]
    model_root = PATHS["models"] / "master_v9" / spec["folder"]
    log_root = PATHS["logs"] / "master_v9" / spec["folder"]
    model_root.mkdir(parents=True, exist_ok=True)
    log_root.mkdir(parents=True, exist_ok=True)

    print("\n" + "=" * 72)
    print(
        f"Training {model_key.upper()} (v9) | lyap={spec['use_lyapunov']} "
        f"| future={spec['use_future']} | blind={spec['blind_features']} "
        f"| lagrangian={spec.get('use_lagrangian', False)} "
        f"| shield={spec.get('shield', False)} "
    ...
    try:
        ...
        print(f"[DONE] {model_key} v9 training completed.")
    except KeyboardInterrupt:
        model.save(model_root / "interrupted_model")
        print(f"[INTERRUPTED] {model_key} v9 checkpoint saved.")
    finally:
        train_env.close()
        eval_env.close()
...
def _run_parallel(model_keys: List[str], max_workers: int, gpu_ids: Optional[List[int]] = None):
    ...
    print(f"\n[PARALLEL v9] Launching {len(remaining)} models, max {max_workers} concurrent")
    if gpu_ids:
        print(f"[PARALLEL v9] GPU pool: {gpu_ids}")
    ...
    while remaining or running:
        while remaining and len(running) < max_workers:
            ...
            log_file = PATHS["logs"] / "master_v9" / f"{key}_parallel.log"
            ...
            print(f"[PARALLEL v9] Started {key.upper()} (PID {proc.pid}){gpu_tag}  -> {log_file}")
        ...
        for key in list(running):
            ...
            if ret is not None:
                ...
                if ret == 0:
                    ...
                    print(f"[PARALLEL v9] {key.upper()} finished successfully")
                else:
                    failed.append(key)
                    print(f"[PARALLEL v9] {key.upper()} FAILED (exit code {ret})")

        if running:
            time.sleep(15)

    print("\n" + "=" * 72)
    print(f"[PARALLEL v9] Completed: {completed}")
    if failed:
        print(f"[PARALLEL v9] FAILED:    {failed}")
        sys.exit(1)
    print("All parallel v9 trainings finished.")
    print("=" * 72)


def main():
    parser = argparse.ArgumentParser(
        description="Train all ABR RL models v9 (paper-ready final)."
    )
    parser.add_argument("--all", action="store_true", help="Train all v9 models.")
    parser.add_argument(
        "--models",
        type=parse_model_list,
        default=["proposed", "proposed_shielded"],
    ...
    if args.parallel > 1 and len(order) > 1:
        ...
        print("\n" + "=" * 72)
        print("All requested v9 trainings finished.")
        print("=" * 72)
